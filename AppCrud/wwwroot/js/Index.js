...
function MostrarEmpleados() {
    fetch("/Home/ListaEmpleados/").then(response => {
        ...
        if (responseJson.length > 0) {
            ...
            responseJson.forEach((emp) => {
                $("#tablaEmpleados tbody").append(
                    $("<tr>").append(
                        $("<td>").text(emp.nombreCompleto),
                        $("<td>").text(emp.refDepartamento.nombre),
                        $("<td>").text(emp.sueldo),
                        $("<td>").text(emp.fechaContrato),
                        $("<td>").append(
                            $("<button>").addClass("btn btn-primary btn-sm boton-editar-empleado").text("Editar").data("dataEmpleado", emp),
                            $("<button>").addClass("btn btn-danger btn-sm ms-2 boton-eliminar-empleado").text("Eliminar").data("dataEmpleado", emp)
                        )
                    )
                )
            })
...
document.addEventListener("DOMContentLoaded", function () {
    ...
    fetch("/Home/ListaDepartamentos/").then(response => {
        ...
        .then(responseJson => {
            if (responseJson.length > 0) {
                responseJson.forEach(dep => {
                    $("#cboDepartamento").append(
                        $("<option>").append(dep.idDepartamento).text(dep.nombre)
                    )
                })
            }
        })
    ...
    if (_modeloEmpleado.idEmpleado == 0) {
        fetch("/Home/CrearEmpleado/", {
            ...
            if (responseJson.valor) {
                ...
                Swal.fire("Done!", "Empleado creado", "success");
                MostrarEmpleados();
            } else {
                Swal.fire("Oh no", "El empleado no se pudo crear", "error")
            }
        ...
        fetch("/Home/EditarEmpleado/", {
            ...
            if (responseJson.valor) {
                $("#modalEmpleado").modal("hide");
                Swal.fire("Done!", "Empleado actualizado", "success");
                MostrarEmpleados();
            } else {
                Swal.fire("Oh no", "El empleado no se pudo actualizar", "error")
            }
    ...
    Swal.fire({
        ...
        if (result.isConfirmed) {
            fetch(`/Home/EliminarEmpleado?idEmpleado=${_empleado.idEmpleado}`, {
                ...
                if (responseJson.valor) {
                    Swal.fire("Done!", "Empleado eliminado", "success");
                    MostrarEmpleados();
                } else {
                    Swal.fire("Oh no", "El empleado no se pudo eliminar", "error")
                }
